...
seajs.config({
	...
	alias:{
		...
		'jquery_html5Validate':'lib/jquery-html5Validate',
        'jquery_validate':'lib/jquery.validate.min',
        'bootstrap_table':'lib/bootstrap-table.min',
        'bootstrap_table_CN':'lib/bootstrap-table-zh-CN.min',
		'dialog':'lib/dialog-min',
		'cookie':'lib/jquery.cookie',
		'common':'lib/common',
		'autocomplete':'lib/jquery.autocompleter.min',
		'bootstrap':'lib/bootstrap.min',
		'showLoading':'lib/jquery.showLoading.min'
	},
	debug: true,
	preload: ['jquery','interface/interfacesConfig'],
	map: [
          [ /^(.*\.(?:css|js))(.*)$/i, '$1?2015060444' ]
    ]
});
seajs.use('jquery',function(){
	...
	seajs.use('jquery_html5Validate');
	//
    seajs.use('cookie');
    seajs.use('dialog');
	seajs.use("common");
/*	if(_page.indexOf("Login")==-1){
    	seajs.use("lib/head");
    }*/
	if(!_page){return false};
	seajs.use('bootstrap');
	seajs.use('showLoading');
    seajs.use("public/pagerTemplate");
    ...
    seajs.use("modules/"+_page+"/template");
    seajs.use(['jquery_validate',"common","modules/"+_page+"/template"],function(){
   //     console.info('table_utils,common,template 加载完毕......');
        seajs.use("modules/"+_page+"/init");
    });
